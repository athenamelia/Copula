...
# simulate n = 1000 for the new test set
set.seed(4321)
n <- 10000
x_mu <- c(0, 0, 0)
x_sigma <- matrix(c(1,.8,.7,
...
nodeA_par <- get_par(est_nodeA)
density_nodeA <- get_loglik(est_nodeA, U_test, nodeA_par)
nodeB_par <- get_par(est_nodeB)
density_nodeB <- get_loglik(est_nodeB, U_test, nodeB_par)
density_mix <- density_nodeA * 0.5 + density_nodeB * 0.5
...
# Gumbel tree - type 1
# For the Gumbel copula, tau must be >= 0. Replacing negative values by 0.Error in optim(par = par, fn = get_loglik, U = U, nac_Node = nac_Node,  : non-finite value supplied by optim

...
x = rHAC(100, HAC)
est.obj = estimate.copula(x, method = 2, hac = HAC, epsilon = 0.1)
est.obj

# our similated data
rtree <- list(list(list("U2", "U9", 1.5), list("U6", "U7", 7.5), 0.7), 
     list(list("U10", "U11", 1.3), list(list(list(list("U1", "U3", 5.25), "U8", 3.9), "U4", 2.7), "U5", 1.8), 0.31), 0)
model <- hac(type = 1, tree = rtree) 
# U_sample <- rHAC(100, model)
# result <- estimate.copula(U_sample, method = 2, hac = model, epsilon = 0.1)
```
...
# Frank tree - type 5
tree1 <- list(list(list("U2", "U9", 1.5), list("U6", "U7", 7.5), 0.7), 
     list(list("U10", "U11", 1.3), list(list(list(list("U1", "U3", 5.25), "U8", 3.9), "U4", 2.7), "U5", 1.8), 0.31), 0)
HAC1 <- hac(type = 5, tree = tree1) 
est.tree1 <- estimate.copula(U_pobs, method = 2, hac = HAC1, epsilon = 0.1)
est.tree1

# Gumbel tree - type 1
tree2 <- list(list(list("U6", "U7", 2), list("U2", "U9", 1.15), 1.05),
              list(list(list(list(list(list("U1", "U8", 1.8), "U3", 1.6), "U4", 1.27), "U5", 1.18), "U10", 1.13), "U11", 1.05),
              1.15)
HAC2 <- hac(type = 1, tree = tree2) 
est.tree2 <- estimate.copula(U_pobs, method = 2, hac = HAC2, epsilon = 0.1)
est.tree2

# Clayton tree - type 3
tree3 <- list(list(list("U11", "U9", 0.21), "U10", 0.11), 
              list(list(list(list(list("U1", "U8", 1.09), "U3", 0.86), "U4", 0.46), "U5", 0.34),
                    list(list("U6", "U7", 2.05), "U2", 0.13), 0), 0)

HAC3 <- hac(type = 3, tree = tree3) 
est.tree3 <- estimate.copula(U_pobs, method = 2, hac = HAC3, epsilon = 0.1)
est.tree3

# method 1
est_hac_frank <- estimate.copula(U_pobs, type = 5) # not work if use U
est_hac_frank
plot(est_hac_frank)

est_hac_gumbel <- estimate.copula(U_pobs, type = 1) # not work if use U
est_hac_gumbel
plot(est_hac_gumbel)

est_hac_clayton <- estimate.copula(U_pobs, type = 3) # not work if use U
est_hac_clayton

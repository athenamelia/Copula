...
test_that("estimate_par works, with subcopulas, independence copula", {
  ...
  est_node <- estimate_par(new_node, U)

  expect_equal(get_par(est_node), c(0.03723496, 0.07648524), tolerance = 0.001)
})
...
test_that("estimate_par works, with subcopulas, simulate data from Clayton copula", {
  ...
  expect_equal(get_par(est_node), c(13.010, 13.647), tolerance = 0.001)
})
...
test_that("estimate_par works, with subcopulas, simulate data from Gumbel copula", {
  ...
  expect_equal(get_par(est_node), c(11.211, 11.071), tolerance = 0.001)
})
...
test_that("estimate_par works, with NULL and subcopulas", {
  ...
  est_node <- estimate_par(nac_node_full, U)

  expect_equal(get_par(est_node), c(19.917, 8.416, 10.222, 8.383, 10.414, 9.679, 11.893), tolerance = 0.001)
})

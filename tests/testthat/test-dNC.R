...
test_that("dNC works, no children", {
    ...
    expect_equal(dNC(U, nlist_no_children, family = 'Clayton'), density_no_children)
})

test_that("dNC works, with NULL", {
    ...
    expect_equal(dNC(U, nlist_null, family = 'Frank'), density_null)
})

test_that("dNC works, normal listing structure", {
    tau <- c(0.05, 0.1) # Kendall's tau
    th <- iTau(archmCopula("Clayton"), tau = tau) # corresponding parameters
    ...
    expect_equal(dNC(U, nlist, family = 'Clayton'), density_test)
})


test_that("dNC works, complicated nesting structure", {
    ...
    # check
    expect_equal(dNC(U, nlist_complicated, family = 'Clayton'), d_test)
})

...
nlist1 <- list(family = "Clayton", th1[1], NULL, list(list("Frank", th1[2], 1, list(list("Clayton", th1[3], 2:3))), # NAC structure
                                                        list("Frank", th1[4], 4, list(list("Frank", th1[5], 5:6))),
                                                        list("Clayton", th1[6], 7, list(list("Clayton", th1[7], 8:11)))))
...
nlist2 <- list("Clayton", th2[1], 1, list(list("Frank", th2[2], 2:3), # NAC structure
                                            list("Frank", th2[3], 4, list("Clayton", list(th2[4], 5:11)))))
...
param3 <- c(.1, .1, .1, .1, .1)
optim_results_3 <- optim(par = param3,
                         fn = loglikNC,
                         U = U,
                         naclist = nlist3,
                         method ="BFGS",
...
# maually
th_initial <- c(0.1, 0.2, 0.5, 0.8, 0.3) # Kendall's tau
empirical_taus <- cor(U[,8:11], method = "kendall")

# figure out how to make indexing below more generic
tau_init <- mean(c(empirical_taus[1, 2:4], empirical_taus[2, 3:4], empirical_taus[3, 4:4]))
th <- iTau(archmCopula(family = "Clayton"), tau = tau_init)

# if th below limits for this copula family/dimension, truncate
# (e.g. here th = -0.07 but a 3d clayton copula can't handle negative th, so set to 0.01 or something)

e_taus <- cor(U[,c(6,7)], method = "kendall")
tau_init <- mean(c(e_taus[1, 2]))
list_test <- list("Clayton", th[5], 6:7)
th_test <- iTau(archmCopula(family = "Clayton"), tau = tau_init)
test_result <- optim(par = th_test,
                     fn = loglikNC,
                     U = U,
                     naclist = list_test,
                     method ="BFGS",
...
# Rosenblatt transform plot
U_test <- cCopula(U[,c(6,7)], copula = claytonCopula(exp(test_result$par)-1), dim = 2)
cc <- claytonCopula(exp(test_result$par)-1, dim = 2)
U_1 <- rCopula(n = 1000, copula = cc)
plot(U_test)
plot(U_1)
...
nesting_list1 <- list("Clayton", nesting_th[1], 1:3)
nesting_list2 <- list("Frank", nesting_th[1], 1, list(list("Clayton", nesting_th[2], 2:3)))
nesting_list3 <- list("Clayton", nesting_th[1], 2, list(list("Gumbel", nesting_th[2], c(1,3))))
nesting_list4 <- list("Frank", nesting_th[1], 3, list(list("Clayton", nesting_th[2], 1:2)))
...
optim_results1 <- optim(par = param1,
                       fn = loglikNC,
                       U = U,
                       naclist = nesting_list1,
                       method ="BFGS",
                       control = list(fnscale = -1))
...
new_nesting_list1 <- updatePars(optim_results1$par, nesting_list1, current_index = 1)[[1]]
new_nesting_list2 <- updatePars(optim_results2$par, nesting_list2, current_index = 1)[[1]]
new_nesting_list3 <- updatePars(optim_results3$par, nesting_list3, current_index = 1)[[1]]
new_nesting_list4 <- updatePars(optim_results4$par, nesting_list4, current_index = 1)[[1]]
...
optim_results # weights not optimized

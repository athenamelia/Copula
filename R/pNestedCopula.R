...
pNC <- function(U, naclist) {
  ...
  if (length(naclist) == 3) {
    ...
    cc <- claytonCopula(naclist[[1]], dim = length(naclist[[2]])+ncol(V))
    nestedCopula <- nestedCopula + pCopula(cbind(U[,naclist[[2]]],V), copula = cc)
  } 
  
  else if (length(naclist) == 2) {
    cc <- claytonCopula(naclist[[1]], dim = length(naclist[[2]]))
    nestedCopula <- nestedCopula + pCopula(U[,naclist[[2]]], copula = cc)
  }
  return(nestedCopula)
}
...
p_test_null == pNC(U[,1:4], nlist_null)
...
p_test_verylong == pNC(U[,1:11], nlist_very_long)
